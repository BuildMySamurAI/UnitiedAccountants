...
export default async function StaffBulkMessagesPage() {
  const supabase = await supabaseServer();
  const recipients = await getBulkMessageRecipients(supabase);

  return (
    <>
      <ConsoleTopBar searchAction="/staff" crumbs={[{ label: "Bulk messages" }]} />
      <div className="wrap">
        <h2 className="page">Bulk messages</h2>
        <p className="sub">Filter your assigned clients by tag, then send one text or email to everyone selected. Clients marked opted-out are skipped automatically.</p>
